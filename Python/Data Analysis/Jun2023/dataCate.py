...
def loadDataCate(directory , dataCateName):
    ...
    file_path = os.path.join(directory , dataCateName + '.xlsx')
    print(f"file_path: ", file_path)
    if os.path.exists(file_path):
        print('Excel files exist in folder')
        excel_files = glob.glob(file_path)
    else:
        print(f'Excel files do not exist in folder {0}', file_path)
    ...
    try:
        ...
        df = pd.read_excel(file_path, sheet_name='Sheet1', usecols=[0,2,3])
        return df
        pass
...
if __name__ == '__main__':
    ...
    df_cate = loadDataCate(datacate_dict, datacate_filename)
    print(df_cate)
    # print("TESTING")
